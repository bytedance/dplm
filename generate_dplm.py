...
def initialize_generation(num_seqs, length, tokenizer, device, cond_seq=None, args=None):
    seq = ['<mask>'] * length
...
def generate(args):
    model = DiffusionProteinLanguageModel.from_pretrained(args.model_name, from_huggingface=False)
    ...
    for seq_len in args.seq_lens:
        ...
        input_tokens = initialize_generation(args.num_seqs, seq_len, tokenizer, device)
        partial_mask = input_tokens.ne(model.mask_id)
        with torch.cuda.amp.autocast():
            outputs = model.generate(input_tokens=input_tokens, tokenizer=tokenizer,
                                        max_iter=max_iter,
