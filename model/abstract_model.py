...
class AbstractModel(pl.LightningModule):
    def __init__(self,
        ...
        self.save_ckpt_list = [] 
        self.save_top_k = save_top_k # save top 1 by default
    ...
    def optimizer_step(
        ...
        super().optimizer_step(
            epoch, batch_idx, optimizer, optimizer_idx, optimizer_closure, on_tpu, # using_native_amp, 
            using_lbfgs
        )
        self.step += 1
    ...
    def load_checkpoint(self, from_checkpoint):
        ...
        if 'state_dict' in state_dict:
            ...
            wxy = 1
            #self.load_weights(self, state_dict)
        elif 'model' in state_dict:
            state_dict = state_dict['model']
            self.load_weights(self.model, state_dict)
        else:
            raise NotImplementedError
    ...
    def save_checkpoint(self, save_path, save_info: dict = None) -> None:
        ...
        if not self.save_weights_only:
            ...
            state_dict['save_ckpt_list'] = self.save_ckpt_list
            ...
            if dist.get_rank() == 0:
                ...
                if best_value:
                    if mode == "min" and now_value < best_value or mode == "max" and now_value > best_value:
                        ...
                        self.save_checkpoint(save_path=os.path.join(self.save_path, 'best.pt'), save_info=save_info)

                else:
                    setattr(self, "best_value", now_value)
                    self.save_checkpoint(save_path=os.path.join(self.save_path, 'best.pt'), save_info=save_info)
                self.save_checkpoint(save_path=os.path.join(self.save_path, f"{mode}_{now_value}.pt"), save_info=save_info)
                self.save_ckpt_list.append(float(now_value))
                if len(self.save_ckpt_list) > self.save_top_k:
                    if mode == 'max':
                        self.save_ckpt_list = sorted(self.save_ckpt_list, reverse=True)
                    else:
                        self.save_ckpt_list = sorted(self.save_ckpt_list)
                    to_be_del = self.save_ckpt_list.pop()
                    try:
                        os.remove(os.path.join(self.save_path, f"{mode}_{to_be_del}.pt"))
                    except:
                        print('No file ', os.path.join(self.save_path, f"{mode}_{to_be_del}.pt"))
                self.save_checkpoint(save_path=os.path.join(self.save_path, 'last.pt'), save_info=save_info)
    ...
    def log_info(self, info: dict,
        ...
        if getattr(self, "logger", None) is not None:
            ...
            self.log_dict(info, prog_bar=prog_bar, logger=logger, on_step=on_step, on_epoch=on_epoch)
    ...
    def configure_optimizers(self):
        ...
        # optimizer_grouped_parameters = [
        #     {'params': [p for n, p in self.trainer.model.parameters() if not any(nd in n for nd in no_decay)],
        #      'weight_decay': weight_decay},
        #     {'params': [p for n, p in self.trainer.model.parameters() if any(nd in n for nd in no_decay)],
        #      'weight_decay': 0.0}
        # ]
        
        self.optimizer = torch.optim.AdamW(self.trainer.model.parameters(),
                                           weight_decay=weight_decay,
                                           lr=self.lr_scheduler_kwargs['init_lr'],
                                           **self.optimizer_kwargs)
