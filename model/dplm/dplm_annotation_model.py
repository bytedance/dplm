...
@register_model
class DPLMAnnotationModel(DPLMBaseModel):
    def __init__(self, anno_type: str, **kwargs):
        """
        Args:
            anno_type: one of EC, GO, GO_MF, GO_CC
            **kwargs: other parameters for DPLMBaseModel
        """
    ...
    def validation_epoch_end(self, outputs):
        ...
        log_dict = {"valid/valid_f1_max": f1_max,
                    "valid/valid_loss": torch.cat(self.all_gather(outputs), dim=-1).mean(),
                    # "valid_aupr": aupr
                    }
        
        self.log_info(log_dict)
        self.reset_metrics("valid")
        #self.check_save_condition(log_dict["valid/valid_f1_max"], mode="max")