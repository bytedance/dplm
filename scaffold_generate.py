...
def get_motif(PDB_ID):
    ...
    pdb_clean_path = os.path.join(
        "data-bin/scaffolding-pdbs/" + str(PDB_ID) + "_clean.pdb"
    )
...
def get_initial(args, tokenizer, pdb, min_len, max_len, device):
    ...
    for i in range(num):
        ...
        seq = (
            ["<mask>"] * (motif_start + 1)
            + list(motif)
            + ["<mask>"] * (length - motif_start - 1 - len(motif))
        )
...
def generate(args, saveto):
    if args.structure_cond:
        model = ConditionalDPLM.from_pretrained(args.model_name)
    ...
    for pdb in start_idx_dict.keys():
        ...
        batch, start_idxs_list, end_idxs_list, scaffold_lengths_list = get_initial(
            args, tokenizer, pdb, min_len, max_len, device
        )
        partial_mask = (
        ...
        if args.structure_cond:
            ...
            extend_coords = (
                torch.zeros(extend_size).fill_(float("nan")).type_as(motif_coords)
            )
            coords_extend_mask = partial_mask.unsqueeze(-1).unsqueeze(-1)
            extend_coords.masked_scatter_(coords_extend_mask, motif_coords)
            batch_struct["coords"] = extend_coords
            batch_struct["motif_mask"] = partial_mask
            batch_struct["prev_tokens"] = batch["input_ids"]
        ...
        output_results = [
            "".join(seq.split(" "))
            for seq in tokenizer.batch_decode(output_tokens, skip_special_tokens=True)
        ]
        ...
        save_df = pd.DataFrame(
            list(zip(strings, start_idxs_list, end_idxs_list, scaffold_lengths_list)),
...
def main():
    ...
    parser.add_argument(
        ...
        help="If using cond-task=scaffold, provide start and end indexes for motif being scaffolded\
                             If defining multiple motifs, supply the start and end -idx motif as a new argument\
                              ex: --start_idx 3 --end_idx 10 --start_idx 20 --end_idx 25\
                              ...
                              this code should be --start_idx 1 and --end_idx 6",
    )
    parser.add_argument("--end_idxs", type=int, action="append")
    parser.add_argument(
        "--scaffold_min", type=int, default=50, help="Min scaffold len "
    )
    parser.add_argument(
        "--scaffold_max",
        type=int,
    ...
    parser.add_argument("--structure_cond", action="store_true", default=False)
