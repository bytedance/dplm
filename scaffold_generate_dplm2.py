...
def generate(args, saveto):
    model = MultimodalDiffusionProteinLanguageModel.from_pretrained(args.model_name)
    ...
    for ori_pdb_name, pdb_name in motif_name_mapping.items():
        ...
        batches, start_idxs_list, end_idxs_list, scaffold_lengths_list = (
            get_initial_dplm2(
                args,
                list(aa_seq),
                struct_seq.split(","),
        ...
        for iteration in range(1, args.sampling_each_seq + 1):
            ...
            aa_strings = [
                "".join(seq.split(" "))
                for seq in tokenizer.batch_decode(aa_tokens, skip_special_tokens=True)
            ]
            struct_strings = [
                ",".join(seq.split(" "))
                for seq in tokenizer.batch_decode(
            ...
            save_df.to_csv(
                os.path.join(scaffold_info_path, f"{ori_pdb_name}.csv"), index=False
            )
...
def save_results(
    ...
    if save_pdb:
        ...
        for header, aatype_str, struct_tokens_str in zip(
            ...
            aatype_tensor, struct_tokens_tensor = struct_tokenizer.string_to_tensor(
            ...
            struct_tokenizer.output_to_pdb(decoder_out, output_dir=pdb_save_dir)
...
def main():
    ...
    parser.add_argument("--sampling_strategy", type=str, default="annealing@2.0:1.0")
    parser.add_argument("--max_iter", type=int, default=500)
    parser.add_argument("--batch_size", type=int, default=100)
    parser.add_argument("--model_name", type=str, default="airkingbd/dplm2_650m")
    parser.add_argument(
        "--motif_aa", type=str, default="./data-bin/scaffolding-pdbs/aa_seq.fasta"
    )
    parser.add_argument(
        "--motif_struct",
        type=str,
