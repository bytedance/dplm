...
class EsmForDPLM2Bit(EsmForMaskedLM):
    def __init__(self, config, dropout=0.1, codebook_embed_dim=13):
        ...
        # aatype output head
        self.lm_head = EsmLMHead(config) # do not change the name for loading DPLM weights.
        # struct output head
        struct_config = deepcopy(config)
        # vocab size is equal to codebook_embed_dim * 2 for binary classification.
        struct_config.vocab_size = getattr(config, "codebook_embed_dim", codebook_embed_dim) * 2
        ...
        self.quant2emb = nn.Linear(
            getattr(config, "codebook_embed_dim", codebook_embed_dim), config.hidden_size
        )
