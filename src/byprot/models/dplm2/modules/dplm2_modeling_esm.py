...
class ModifiedRotaryEmbedding(RotaryEmbedding):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, type_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        self._cos_cached, self._sin_cached = self._update_cos_sin_tables(k, type_ids=type_ids, seq_dimension=-2)
...
class ModifiedEsmSelfAttention(EsmSelfAttention):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.rotary_embeddings = ModifiedRotaryEmbedding(dim=self.attention_head_size)
...
class ModifiedEsmModel(EsmModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            ...
            attention_mask=input_ids.ne(self.config.pad_token_id), #attention_mask,
...
class EsmForDPLM2(EsmForMaskedLM):
    ...
    def _get_resized_embeddings(
        ...
        ptrn_emb_mean = old_embeddings.weight.data.mean(dim=0).repeat(new_num_tokens - old_num_tokens, 1)
        ptrn_emb_std = old_embeddings.weight.data.std(dim=0).repeat(new_num_tokens - old_num_tokens, 1)
        new_embeddings.weight.data[n:, :] = torch.normal(mean=ptrn_emb_mean, std=ptrn_emb_std)
